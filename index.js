...
async function startjobotz() {
    const jobotz = jobotzConnect({
        ...
        browser: ['THE JO BOT MD', 'Safari','1.0.0'],
        auth: state
    })
    ...
    jobotz.ev.on('group-participants.update', async (anu) => {
        ...
        try {
            ...
            for (let num of participants) {
                ...
                if (anu.action == 'add') {
                  jobotz.sendMessage(anu.id, { image: { url: pp_user }, contextInfo: { mentionedJid: [num] }, caption: `Yokoso ${num.split("@")[0]}\n\nWelcome To${metadata.subject}\n\nBy: FaiRuzz +62` })
                    
                } else if (anu.action == 'remove') {
                  jobotz.sendMessage(anu.id, { image: { url: pp_user }, contextInfo: { mentionedJid: [num] }, caption: `Sayonara ${num.split("@")[0]}\n\nLeave To ${metadata.subject}\n\nBy: FaiRuzz +62` })
